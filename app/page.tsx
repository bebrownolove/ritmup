...
type Modal = 'food' | 'weight' | 'activity' | null;
...
function calculateStreak(days: Record<string, Day>) {
  let offset = days[todayKey()]?.foods.length ? 0 : -1;
  let streak = 0;
  while (days[dayOffsetKey(offset)]?.foods.length) { streak += 1; offset -= 1; }
  return streak;
}
...
export default function Home() {
  ...
  const streak = calculateStreak(store.days);
  ...
  return (
    <main className="app-shell">
      <section className="phone-frame">
        {tab === 'today' && <>
          <header className="topbar"><div><p className="eyebrow">{dateLabel}</p><h1>{greeting}, {store.name}</h1></div><div className={`streak ${day.foods.length ? 'streak-live' : ''}`} aria-label={`Серия: ${streak} дней`}><span>🔥</span><strong>{streak}</strong></div></header>
          ...
          <section className={`coach-card ${day.foods.length ? 'coach-happy' : ''}`} aria-live="polite"><div className="coach-face"><span>{day.foods.length ? '🔥' : '🌱'}</span></div><p><strong>{day.foods.length ? 'Сегодняшний огонёк горит!' : 'Начнём без спешки'}</strong><small>{day.foods.length ? 'День уже засчитан — вечером ничего нажимать не нужно.' : 'Первая запись еды автоматически продолжит твою серию.'}</small></p></section>
          ...
          <div className="section-heading today-heading"><h2>Сегодня</h2><span>{[day.weight !== null, day.foods.length > 0].filter(Boolean).length} из 2 записано</span></div>
          <section className="task-grid">
            ...
            <article className="task-card wide auto-card"><span className="task-icon">🌙</span><div><small>Автоматический итог</small><strong>Новый день начнётся сам</strong></div><span className="auto-badge">AUTO</span></article>
          </section>
        </>}
      ...
      {celebrate && <div className="celebration" role="status" aria-label="Сегодняшний день засчитан"><span>•</span><span>★</span><span>•</span><span>🔥</span><strong>День засчитан!</strong><span>•</span><span>★</span><span>•</span></div>}
      {modal && <ModalShell title={modal === 'food' ? 'Добавить еду' : modal === 'weight' ? 'Утренний вес' : 'Активность'} onClose={() => setModal(null)}>
...
function History({ store }: { store: Store }) {
  ...
  return <section className="inner-page"><p className="eyebrow">ТВОЙ ПРОГРЕСС</p><h1>История</h1><div className="history-hero"><span>🔥</span><div><strong>{calculateStreak(store.days)}</strong><small>дней подряд</small></div></div><div className="history-list">{entries.map(([date, day]) => { const total = day.foods.reduce((sum, food) => sum + food.calories, 0); return <article key={date}><time>{new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'long' }).format(new Date(`${date}T12:00:00`))}</time><div><strong>{total} ккал</strong><small>{day.weight ? `${day.weight} кг · ` : ''}{day.activity} активных</small></div><span>{day.foods.length ? '✓' : '•'}</span></article>; })}</div>{entries.length === 0 && <p className="empty-state">Первый день появится здесь после записи.</p>}</section>;
}
