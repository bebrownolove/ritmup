  ...
  return <main className="onboarding-shell">
    <section className="onboarding-card pop-in">
      <div className="onboarding-top">
        <div className="onboarding-mascot" aria-hidden>{step===2?"🎉":"🔥"}</div>
        <div className="onboarding-progress" aria-label={`Шаг ${step+1} из 3`}><i style={{width:`${(step+1)*33.34}%`}}/></div>
      </div>

      {step===0&&<div className="onboarding-step" key="goal">
        <p className="eyebrow">ШАГ 1 ИЗ 3</p><h1>К чему идём?</h1><p className="muted">Подберём ориентир по калориям. Его всегда можно изменить.</p>
      ...
      {step===1&&<div className="onboarding-step" key="body">
        <p className="eyebrow">ШАГ 2 ИЗ 3</p><h1>Пара слов о тебе</h1><p className="muted">Это нужно только для примерного расчёта расхода энергии.</p>
        <div className="onboarding-form">
          ...
          <label className="wide">Обычная активность<select value={activity} onChange={e=>setActivity(e.target.value)}>{Object.entries(ACTIVITY).map(([key,item])=><option key={key} value={key}>{item.label}</option>)}</select></label>
        </div>
        {!result&&height&&weight&&birthYear&&<p className="onboarding-error">Проверь введённые значения.</p>}
        <div className="onboarding-actions"><button className="ghost" onClick={()=>setStep(0)}>Назад</button><button className="primary" disabled={!result} onClick={()=>setStep(2)}>Посчитать</button></div>
      </div>}

      {step===2&&result&&<div className="onboarding-step result-step" key="result">
      ...
      {step<2&&<button className="onboarding-skip" disabled={busy} onClick={()=>void skip()}>Настроить позже</button>}
      {error&&step<2&&<p className="onboarding-error">{error}</p>}
    </section>
  </main>;
}
