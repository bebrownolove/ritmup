...
function PersonRow({person,action}:{person:Person;action?:React.ReactNode}) {const hasMetrics=person.status==="accepted"&&(person.sharesWeight||person.sharesCalories||person.sharesSteps);return <div className={`person${hasMetrics?" with-metrics":""}`}><CharacterAvatar value={person.avatarConfig} size="small" label={`Персонаж ${person.name}`}/><div><b>{person.name}</b><small>@{person.username??"без-ника"}</small></div>{action&&<div className="person-action">{action}</div>}{hasMetrics&&<div className="friend-metrics">{person.sharesWeight&&<span>⚖️ <b>{person.sharedWeightKg??"—"}</b> кг</span>}{person.sharesCalories&&<span>🍽️ <b>{person.sharedCalories??"—"}</b> ккал</span>}{person.sharesSteps&&<span>👟 <b>{person.sharedSteps??"—"}</b> шагов</span>}</div>}</div>}
...
function Profile({user,avatar,onAvatarChange,onOpenOnboarding}:{user:{name:string;email:string;username?:string|null};avatar?:Partial<AvatarConfig>|null;onAvatarChange:(value:AvatarConfig)=>void;onOpenOnboarding:()=>void}) {
  const [settings,setSettings]=useState({isDiscoverable:true,shareStreak:true,shareGoalHits:true,shareWorkouts:true,shareWeight:false,shareCalories:false,shareSteps:false}); const [saved,setSaved]=useState(false);
