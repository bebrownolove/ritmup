...
export type HistoryDay = {
  ...
  food:{title:string;calories:number}[];
  workouts:{title:string;minutes:number;calories:number|null}[];
};
...
export function StatsScreen() {
  ...
  const period=<div className="segmented" role="tablist" aria-label="Период">
    {([[7,"Неделя"],[30,"Месяц"],[90,"3 месяца"]] as [number,string][]).map(([value,label])=>
  ...
  return <section className="screen">
    <div className="segmented" role="tablist" aria-label="Вид истории">
      <button role="tab" aria-selected={view==="diary"} className={view==="diary"?"active":""} onClick={()=>setView("diary")}>Дневник</button>
      <button role="tab" aria-selected={view==="stats"} className={view==="stats"?"active":""} onClick={()=>setView("stats")}>Статистика</button>
    </div>
    {period}

    {view==="diary"?<div className="history-days">
      {[...days].reverse().map((day,index)=>{
        const over=day.caloriesEaten>day.calorieGoal;
        const hasMovement=day.workouts.length>0||(day.steps??0)>0||day.activeCalories>0;
        const hasData=day.food.length>0||hasMovement||day.weightKg!==null;
        return <details className="history-day" key={day.date} open={index===0&&hasData}>
          <summary>
            <span><b>{new Date(`${day.date}T00:00`).toLocaleDateString("ru",{weekday:"long",day:"numeric",month:"long"})}</b><small>{hasData?`${spaced(day.caloriesEaten)} ккал из ${spaced(day.calorieGoal)}`:"Записей нет"}</small></span>
            <em className={over?"over":""}>{hasData?"Подробнее":"—"}</em><i>›</i>
          </summary>
          <div className="history-day-body">
            <section><h4>🍽️ Питание</h4>{day.food.length?<div className="history-rows">{day.food.map((item,itemIndex)=><p key={`${item.title}-${itemIndex}`}><span>{item.title}</span><b>{spaced(item.calories)} ккал</b></p>)}</div>:<small>Еда не записана</small>}</section>
            <section><h4>🏃 Движение</h4>{hasMovement?<div className="history-rows">{day.steps!==null&&<p><span>Шаги</span><b>{spaced(day.steps)}</b></p>}{day.activeCalories>0&&<p><span>Активная энергия</span><b>{spaced(day.activeCalories)} ккал</b></p>}{day.workouts.map((item,itemIndex)=><p key={`${item.title}-${itemIndex}`}><span>{item.title}</span><b>{item.minutes} мин</b></p>)}</div>:<small>Движение не записано</small>}</section>
            <section><h4>⚖️ Вес</h4>{day.weightKg!==null?<b className="history-weight">{day.weightKg.toFixed(1).replace(".",",")} кг</b>:<small>Вес не записан</small>}</section>
          </div>
        </details>;
      })}
    </div>:<>
      <div className="hero-dark">
        <p className="eyebrow">СРЕДНЕЕ ЗА {range===7?"НЕДЕЛЮ":range===30?"МЕСЯЦ":"3 МЕСЯЦА"}</p>
      ...
      <div className="tiles stats-tiles">
        <div className="tile"><p className="eyebrow">ШАГИ В ДЕНЬ</p><b>{withSteps.length?spaced(Math.round(steps/withSteps.length)):"—"}</b><small>{withSteps.length?`за ${withSteps.length} ${plural(withSteps.length,"день","дня","дней")} с данными`:"нет данных"}</small></div>
        <div className="tile"><p className="eyebrow">ТРЕНИРОВКИ</p><b>{minutes}</b><small>{plural(minutes,"минута","минуты","минут")} за период</small></div>
        <div className="tile" style={{gridColumn:"1/-1"}}><p className="eyebrow">ВЕС</p><b>{weighed.length?`${weighed[weighed.length-1].weightKg!.toFixed(1).replace(".",",")} кг`:"—"}</b><small className={weightChange!==null&&weightChange<0?"up":""}>{weightChange===null?"запиши вес хотя бы дважды":`${weightChange>0?"+":"−"}${Math.abs(weightChange).toFixed(1).replace(".",",")} кг за период`}</small><WeightChart days={days}/></div>
      </div>
    </>}
  </section>;
}
