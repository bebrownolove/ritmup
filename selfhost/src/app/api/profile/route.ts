...
export async function GET(request: Request) {
  ...
  const result = await db.query(
    `select bio, is_discoverable as "isDiscoverable", share_streak as "shareStreak",
            share_goal_hits as "shareGoalHits", share_workouts as "shareWorkouts", timezone
...
export async function PATCH(request: Request) {
  ...
  await db.query(
    `insert into profiles (user_id, bio, is_discoverable, share_streak, share_goal_hits, share_workouts, timezone)
