...
const schema=z.object({date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),caloriesEaten:z.number().int().min(0).max(20000),
  activeCalories:z.number().int().min(0).max(20000),calorieGoal:z.number().int().min(500).max(10000),
...
export async function POST(request:Request) {
  ...
  await db.query(`insert into daily_logs (user_id,log_date,calories_eaten,active_calories,calorie_goal,weight_kg)
    ...
    active_calories=excluded.active_calories,calorie_goal=excluded.calorie_goal,weight_kg=excluded.weight_kg,updated_at=now()`,
    [user.id,l.date,l.caloriesEaten,l.activeCalories,l.calorieGoal,l.weightKg??null]);
